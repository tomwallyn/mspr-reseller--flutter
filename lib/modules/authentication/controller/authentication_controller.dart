...
class AuthenticationController extends StateNotifier<AsyncValue<CustomUser?>> {
  ...
  void checkUser({required String username, required String password}) async {
    ...
    final result = await _authenticationRepository.checkUser(
  ...
  void overrideToken({required String token}) async {
    state = const AsyncValue.loading();

    final result = await _authenticationRepository.checkToken(token: token);
